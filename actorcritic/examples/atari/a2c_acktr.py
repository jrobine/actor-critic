...
def train_a2c_acktr(acktr, env_id, num_envs, num_steps, save_path):
    ...
    # stacking frames inside the subprocesses would cause the frames to be passed between process multiple times
    ...
    if acktr:
        ...
        model.register_losses(layer_collection)

        # optimizer to use for 'cold' updates at start, to prevent initialization errors # TODO
        cold_optimizer = tf.train.MomentumOptimizer(1e-5, 0.9)

        optimizer = ColdStartPeriodicInvUpdateKfacOpt(
            cold_updates=50, cold_optimizer=cold_optimizer, invert_every=10, learning_rate=0.25,
            cov_ema_decay=0.99, damping=0.01, layer_collection=layer_collection, momentum=0.9,
            norm_constraint=0.0001,  # trust region radius
        ...
        try:
            while True:
                ...
                if step % 100 == 0 and step > 0:
                    ...
                    saver.save(session, save_path, step)
                    print('Saved model', step)
...
if __name__ == '__main__':
    ...
    train_a2c_acktr(acktr, env_id, num_envs, num_steps, save_path)
